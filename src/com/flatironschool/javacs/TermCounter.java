...
public class TermCounter {
	 ...
	 * @param paragraphs
	 */
	public void processElements(Elements paragraphs, Set<String> stopWords) {
		for (Node node: paragraphs) {
			processTree(node, stopWords);
		}
	}
	 ...
	 * @param root
	 */
	public void processTree(Node root, Set<String> stopWords) {
		// NOTE: we could use select to find the TextNodes, but since
		// we already have a tree iterator, let's use it.
		for (Node node: new WikiNodeIterable(root)) {
			if (node instanceof TextNode) {
				processText(((TextNode) node).text(), stopWords);
			}
		}
	}
	 ...
	 */
	public void processText(String text, Set<String> stopWords) {
	...
	public static void main(String[] args) throws IOException {
		...
		counter.processElements(paragraphs, null);
		counter.printCounts();
	}
}
